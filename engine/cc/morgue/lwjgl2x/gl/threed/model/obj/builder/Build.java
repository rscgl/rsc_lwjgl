...
public class Build implements BuilderInterface {
	...
	public void addVertexGeometric(float x, float y, float z) {
		...
		// System.err.println("Added geometric vertex " + verticesG.size() + " =
		// " + verticesG.get(verticesG.size() - 1));
	}

	public void addVertexTexture(float u, float v) {
		verticesT.add(new VertexTexture(u, v));
		// System.err.println("Added texture vertex " + verticesT.size() + " = "
	...
	public void addFace(int[] vertexIndices) {
		...
		for (int i = 0; i < vertexIndices.length;) {
			FaceVertex vert = new FaceVertex();

			idx = vertexIndices[i++];
			if (idx < 0) {
				idx += verticesG.size();
			}
			if (idx != EMPTY_VERTEX_VALUE) {
				vert.v = verticesG.get(idx - 1);
			}
			...
			if (idx != EMPTY_VERTEX_VALUE) {
				vert.n = verticesN.get(idx - 1);
			}

			String key = vert.toString();
			FaceVertex fv2 = faceVerticeMap.get(key);
			if (fv2 == null) {
				faceVerticeMap.put(key, vert);
				vert.index = faceVerticeList.size();
				faceVerticeList.add(vert);
			} else {
				vert = fv2;
			}
	...
	// if (null == names) {
	// System.err.println("Build.addMaterialLib: @TODO: Got a mtllib line with
	// null names array - blank group line? (i.e. \"g\\n\" ?)");
	// return;
	// }
	// if (names.length == 1) {
	// System.err.println("Build.addMaterialLib: @TODO: Got a mtllib line with
