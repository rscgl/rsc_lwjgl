package rscgl.client.game.world;
...
public class WorldLoader {
	...
	public boolean loadContainingSector(MudClient3D terrain, int tileX, int tileZ) {
		...
		loadSector(terrain, sectorX, sectorZ);
	...
	public void loadSector(MudClient3D terrain, int sectorX, int sectorZ) {
		...
		loadRequiredLayers(terrain, sectorX, sectorZ, world.getCurrentLayer());
	...
	private void loadRequiredLayers(MudClient3D terrain, int sectorX, int sectorZ, int currentLayer) {
		...
		loadLayer(terrain, sectorX, sectorZ, currentLayer, true);

		if (currentLayer == 0) {

			// Load upper storeys (they should be visible from the ground floor)
			loadLayer(terrain, sectorX, sectorZ, 1, false);
			loadLayer(terrain, sectorX, sectorZ, 2, false);

			// Set the active sectors back to the current layer
			//setCurrentSector(sectorX, sectorZ, currentLayer);
			world.setSector(0, Resources.loadSector(sectorX - 1, sectorZ - 1, currentLayer));
			world.setSector(1, Resources.loadSector(sectorX, sectorZ - 1, currentLayer));
			world.setSector(2, Resources.loadSector(sectorX - 1, sectorZ, currentLayer));
			world.setSector(3, Resources.loadSector(sectorX, sectorZ, currentLayer));
	...
	private void loadLayer(MudClient3D terrain, int sectorX, int sectorZ, int layer, boolean isCurrentLayer) {
		...
		if (isCurrentLayer) {
			...
			for (int x = 0; x < World.NUM_TILES_X; x++) {
				for (int z = 0; z < World.NUM_TILES_Z; z++) {
					...
					if (world.getGroundTextureOverlay(x, z) > 0 && Resources.getTileDef(world.getGroundTextureOverlay(x, z) - 1).getType() == TileDef.TYPE_BRIDGE) {
						elevation = 0;
					} else if (world.getGroundTextureOverlay(x - 1, z) > 0 && Resources.getTileDef(world.getGroundTextureOverlay(x - 1, z) - 1).getType() == TileDef.TYPE_BRIDGE) {
						elevation = 0;
					} else if (world.getGroundTextureOverlay(x, z - 1) > 0 && Resources.getTileDef(world.getGroundTextureOverlay(x, z - 1) - 1).getType() == TileDef.TYPE_BRIDGE) {
						elevation = 0;
					} else if (world.getGroundTextureOverlay(x - 1, z - 1) > 0 && Resources.getTileDef(world.getGroundTextureOverlay(x - 1, z - 1) - 1).getType() == TileDef.TYPE_BRIDGE) {
						elevation = 0;
					}
			...
			for (int x = 0; x < World.NUM_TILES_X - 1; x++) {
				for (int z = 0; z < World.NUM_TILES_Z - 1; z++) {
					...
					if (world.getGroundTextureOverlay(x, z) > 0) {
						...
						if (tileType1 == 5) {
							if (getDiagonalWalls(x, z) > 0 && getDiagonalWalls(x, z) < 24000) {
								if (getOverlayIfRequired(x - 1, z, groundColour2) != 0xbc614e && getOverlayIfRequired(x, z - 1, groundColour2) != 0xbc614e) {
									groundColour = getOverlayIfRequired(x - 1, z, groundColour2);
									triangleIndex = 0;
								} else if (getOverlayIfRequired(x + 1, z, groundColour2) != 0xbc614e && getOverlayIfRequired(x, z + 1, groundColour2) != 0xbc614e) {
							...
							// Create smooth diagonal lines for road / water
							// edges, etc.
					...
					int i17 = ((world.getGroundElevation(x + 1, z + 1) - world.getGroundElevation(x + 1, z)) + world.getGroundElevation(x, z + 1)) - world.getGroundElevation(x, z);
			...
			for (int x = 1; x < World.NUM_TILES_X - 1; x++) {
				for (int z = 1; z < World.NUM_TILES_Z - 1; z++) {
					...
					if (world.getGroundTextureOverlay(x, z) > 0 && Resources.getTileDef(world.getGroundTextureOverlay(x, z) - 1).getType() == TileDef.TYPE_BRIDGE) {
						...
						int i15 = tmpModel.addUniqueVertex((x + 1) * 128, -world.getGroundElevation(x + 1, z + 1), (z + 1) * 128);
					...
					} else if (world.getGroundTextureOverlay(x, z) == 0 || Resources.getTileDef(world.getGroundTextureOverlay(x, z) - 1).getType() != 3) {
						if (world.getGroundTextureOverlay(x, z + 1) > 0 && Resources.getTileDef(world.getGroundTextureOverlay(x, z + 1) - 1).getType() == TileDef.TYPE_BRIDGE) {
							...
							int j15 = tmpModel.addUniqueVertex((x + 1) * 128, -world.getGroundElevation(x + 1, z + 1), (z + 1) * 128);
						...
						if (world.getGroundTextureOverlay(x, z - 1) > 0 && Resources.getTileDef(world.getGroundTextureOverlay(x, z - 1) - 1).getType() == TileDef.TYPE_BRIDGE) {
							...
							int k15 = tmpModel.addUniqueVertex((x + 1) * 128, -world.getGroundElevation(x + 1, z + 1), (z + 1) * 128);
						...
						if (world.getGroundTextureOverlay(x + 1, z) > 0 && Resources.getTileDef(world.getGroundTextureOverlay(x + 1, z) - 1).getType() == TileDef.TYPE_BRIDGE) {
							...
							int l15 = tmpModel.addUniqueVertex((x + 1) * 128, -world.getGroundElevation(x + 1, z + 1), (z + 1) * 128);
						...
						if (world.getGroundTextureOverlay(x - 1, z) > 0 && Resources.getTileDef(world.getGroundTextureOverlay(x - 1, z) - 1).getType() == TileDef.TYPE_BRIDGE) {
							...
							int i16 = tmpModel.addUniqueVertex((x + 1) * 128, -world.getGroundElevation(x + 1, z + 1), (z + 1) * 128);
		...
		for (int x = 1; x < World.NUM_TILES_X - 1; x++) {
			for (int z = 1; z < World.NUM_TILES_Z - 1; z++) {
				...
				if (i12 > 0) {
					...
					l27 = -l27;
					i28 = -i28;
					if (getDiagonalWalls(x, z) > 12000 && getDiagonalWalls(x, z) < 24000 && getRoofTexture(x - 1, z - 1) == 0) {
						int ai8[] = new int[3];
					...
					} else if (getDiagonalWalls(x, z) > 12000 && getDiagonalWalls(x, z) < 24000 && getRoofTexture(x + 1, z + 1) == 0) {
						int ai9[] = new int[3];
					...
					} else if (getDiagonalWalls(x, z) > 0 && getDiagonalWalls(x, z) < 12000 && getRoofTexture(x + 1, z - 1) == 0) {
						int ai10[] = new int[3];
		...
		// Set the tile x/z so we can use it later to generate 3D tiles.
		// This a section of code that only exists for RSCGL, because jagex didn't store tile x/z coordinates in the Tile class (or anywhere else).
		TileDef tileDef = null;
		Tile tile = null;
		
		for (int i = 0; i < world.getSectors().length; i++) {
			if (world.getSector(i) != null) {
				for (int x = 0; x < Sector.WIDTH; x++) {
					for (int z = 0; z < Sector.DEPTH; z++) {
						tileDef = Resources.getTileDef(world.getGroundTextureOverlay(x, z));
						if (tileDef != null) {
							tile = world.getSector(0).getTile(x, z);
							tile.x = x;
							tile.z = z;
						}
					}
	...
	private boolean isCentreRoof(int x, int z) {
		return getRoofTexture(x, z) > 0 && getRoofTexture(x - 1, z) > 0 && getRoofTexture(x - 1, z - 1) > 0 && getRoofTexture(x, z - 1) > 0;
	}
	...
	private void setGroundTextureOverlays() {
		for (int x = 0; x < World.NUM_TILES_X; x++) {
			for (int z = 0; z < World.NUM_TILES_Z; z++) {
				...
				if (x == 47 && world.getGroundTextureOverlay(x + 1, z) != 250 && world.getGroundTextureOverlay(x + 1, z) != 2) {
					world.setGroundTextureOverlay(x, z, 9);
				} else if (z == 47 && world.getGroundTextureOverlay(x, z + 1) != 250 && world.getGroundTextureOverlay(x, z + 1) != 2) {
					world.setGroundTextureOverlay(x, z, 9);
				} else {
